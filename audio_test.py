...
class Main(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def _update_plot(self):
        self.sc.test_axes = []
        self.sc.fig.clear(True)
        column = len(self.y1['data']) + len(self.y1['data']) + 1
        if column == 1:
            column = 3
        j = 0
        k = 0
        for i in range(len(self.y1['data'])):
            self.sc.test_axes.append(self.sc.fig.add_subplot(column, 1, i + 1))
            self.sc.test_axes[i].plot(self.x1, self.y1['data'][i], label=self.y1['name'][i])
            #self.sc.fig.set_figheight(self.sc.fig.get_figheight()*1.25)
            self.sc.test_axes[i].set_title(self.g1_title)
            mplcursors.cursor(self.sc.test_axes[i])
            j = i
            #self.axes = fig.add_subplot(311)
            #self.axes1 = fig.add_subplot(312)
            #self.axes2 = fig.add_subplot(313)
        ...
        for i in range(len(self.y2['data'])):
            self.sc.test_axes.append(self.sc.fig.add_subplot(column, 1, j + i + 1))
            self.sc.test_axes[j + i].plot(self.x1, self.y2['data'][i], label=self.y2['name'][i])
            #self.sc.fig.set_figheight(self.sc.fig.get_figheight()*1.25)
            self.sc.test_axes[j + i].set_title(self.g2_title)
            mplcursors.cursor(self.sc.test_axes[j + i])
            k = j + i

        self.sc.test_axes.append(self.sc.fig.add_subplot(column, 1, k + 1))
        self.sc.test_axes[k].plot(self.x3, self.y3, 'r')
        #self.sc.fig.set_figheight(self.sc.fig.get_figheight()*1.25)
        self.sc.test_axes[k].set_title(self.g3_title)
        mplcursors.cursor(self.sc.test_axes[k])
